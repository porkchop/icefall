...
This file provides guidance to AI coding agents (Claude Code, Cursor, Copilot, OpenCode, Gemini CLI, Antigravity, etc.) when working with the **icefall** repository.

## Repository overview

icefall was scaffolded from `phasekit`. It uses a phase-gated, audit-first workflow: work progresses through defined phases with explicit approval artifacts and git commits between each. Adapt this section to describe what icefall actually does once the spec is filled in.
...
- `docs/SPEC.md` — what icefall is supposed to do
