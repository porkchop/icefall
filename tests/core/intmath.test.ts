...
import { idiv, imod } from "../../src/core/intmath";

/**
 * Phase 4.A.2 unit tests for the relocated `idiv` / `imod` helpers.
 * Originated in `src/mapgen/math.ts` (Phase 2); relocated to
 * `src/core/intmath.ts` so `src/atlas/**` can consume them without a
 * forbidden cross-layer import. Mirrors the Phase 4.A.1 `uniformIndex`
 * relocation pattern. The original `src/mapgen/math.test.ts` retains
 * coverage via the `src/mapgen/math.ts` re-export shim.
 */
