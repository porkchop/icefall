...
# icefall: Vite + TypeScript + Vitest + Playwright stack.
# Fast checks here; Playwright cross-runtime E2E is too heavy for the
# per-iteration gate and lives in the deploy CI / verification-sprint gate.
npm run lint
npm run typecheck
npm test
...
# below fires every iteration as a reminder that icefall's verify
